...
<xsl:template name="buildGetAvailRequestsForm">
   ...
   <xsl:for-each select="//page:element[@nameId='patronRequests.group']">
      <div class="AvailRequests">
		...
		<!-- WSP added 18 Jan 2012 to eliminate Library Warehouse journal option from request types -->
		<xsl:choose>
		<xsl:when test="contains('Library Warehouse Journals', page:linkText)"></xsl:when>
		<xsl:otherwise>
               <li>
