                ...
                <div class="resultListCellBlock">
                    ...
                    -->
                            ...
                            <xsl:variable name="linkURL">
                                <xsl:value-of select="page:element[@nameId='page.searchResults.contents.line1']/page:URL"/>
                            </xsl:variable>
							...
							<xsl:when test="page:value = 'noItem'">noItem</xsl:when>
								...
								<xsl:choose>
								<xsl:when test="page:value = 'multiHold'">
								<xsl:value-of select="page:label"/>
								</xsl:when>
								<xsl:when test="page:value = 'noItem'">
								</xsl:when>
								<xsl:otherwise>
								<xsl:value-of select="substring-after(page:label,', ')"/>
								</xsl:otherwise>
								</xsl:choose>
                                                </xsl:when>
                                                <xsl:otherwise>
...
<xsl:variable name="VTYPE"><xsl:value-of select="page:value" /></xsl:variable>
<xsl:variable name="VFORMAT"><xsl:value-of select="page:option/page:element[@nameId='page.searchResults.item.type.bibFormat']/page:value" /></xsl:variable>

                     <!-- ## cover image ## -->
                    <xsl:choose>
			 <!-- ## cover image for DVD ## -->
			   <xsl:when test="contains($VFORMAT, 'gm')">
                            <xsl:call-template name="buildDVDResultsCoverImage">
...
<!-- ############################ -->
<!-- ## buildDVDResultsCoverImage ## -->
<!-- ######################################################### -->

<xsl:template name="buildDVDResultsCoverImage">
<xsl:param name="tag"/>
      <div class="resultListCoverImageCell">
...
<!-- ##  VARIABLE TO HOLD LINE 1 OF RETURNED LIST INFO FOR EACH ITEM - HAS BIB_ID IN IT AND IS PASSED TO PERL SCRIPT BELOW ##### -->
   <xsl:variable name="jlines3">
	<xsl:for-each select="page:element[@nameId='titles']">
		<xsl:for-each select="page:option">
			<xsl:for-each select="page:option">
				<xsl:value-of select="concat(page:element[@nameId='page.searchResults.contents.line1']/page:URL,',')"/>
			</xsl:for-each>
		</xsl:for-each>
	</xsl:for-each>
   </xsl:variable>
   ...
   <xsl:for-each  select="page:element[@nameId='page.searchResults.browseBar']">
    ...
    <xsl:variable name="classPosition">
      ...
      <div class="{$classPosition}">
         ...
         <ul>
<!-- #####  LINK TO PERL SCRIPT WITH RESULT LIST LINE 1 INFO (HAS BIB_ID) ##################################  -->
	    <!-- <xsl:choose>
		<xsl:when test="contains($currPage, 'resultsMyList')"> -->
<!-- ####  REMOVED CHOOSE SO LINK BUTTON APPEARS ON BOTH SEARCH RESULTS AND MY LIST PAGES #################### -->

			<li id="QRCodeBVtn">
			<span class="yellowBtnLeft">&#160;</span>
				<a class="yellowBtn" href="http://voyager.falmouth.ac.uk/vwebv/JmyList.cgi?bibId_str='{$jlines3}'">QR Codes</a>
			<span class="yellowBtnRight">&#160;</span>
               ...
               <li>
                  <div class="selectChkBox">
                     ...
                     <xsl:for-each select="page:option[@nameId='page']">
                        <input id="{@nameId}" type="checkbox" name="{@nameId}" value="page" onClick="selectPageCheckbox(document.getElementById('resultsForm'),this)"/>
                        <label for="{@nameId}"><xsl:value-of select="page:text"/></label>
                         <input name="pageIds" type="hidden" value="{page:value}" />
                     </xsl:for-each>
                     <xsl:for-each select="page:option[@nameId='cbxAll']">
                        <input id="{@nameId}" type="checkbox" name="{@nameId}" value="{page:value}" onClick="selectAllCheckbox(document.getElementById('resultsForm'),this)"/>
                        <label for="{@nameId}"><xsl:value-of select="page:text"/></label>
                     </xsl:for-each>
                  </div>
               </li>
...
<a href="http://library.falmouth.ac.uk/863/using-the-library/enquiries/ask-a-librarian/article.asp">
Ask a Librarian</a>
</span>
