...
<!--
**          Product : WebVoyage :: displayStaff
**          Version : 7.2.0
**          Created : 26-OCT-2007
...
<xsl:stylesheet version="1.0"
   ...
   xmlns:fo="http://www.w3.org/1999/XSL/Format">
	...
	<xsl:variable name="currPage">displayStaff</xsl:variable>
	
	<xsl:variable name="displayStaffCSS">
	...
	<xsl:template match="/">
		<xsl:call-template name="buildHtmlPage">
			<xsl:with-param name="myCSS" select="$displayStaffCSS"/>
		</xsl:call-template>
	</xsl:template>
	...
	<xsl:template name="buildContent">
		<xsl:call-template name="buildStaffForm"/>
	</xsl:template>
