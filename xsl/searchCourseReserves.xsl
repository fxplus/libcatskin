...
<!-- Specific Imports -->
<xsl:include href="./contentLayout/cl_searchCourseReserves.xsl"/>
<xsl:include href="./pageFacets/searchFacets.xsl"/>

<!-- Variable Declarations -->
<xsl:variable name="currPage">searchCourseReserve</xsl:variable>

<xsl:variable name="searchCourseReserveCSS">
   <link href="{$css-loc}searchPages.css"         media="all" type="text/css" rel="stylesheet"/>
   <link href="{$css-loc}searchCourseReserve.css" media="all" type="text/css" rel="stylesheet"/>
</xsl:variable>
...
<xsl:template match="/">
	<xsl:call-template name="buildHtmlPage">
		<xsl:with-param name="myCSS"  select="$searchCourseReserveCSS"/>
	</xsl:call-template>		
</xsl:template>
...
<xsl:template name="buildContent">
	...
	<xsl:variable name="formData">
		<xsl:call-template name="buildCourseReservesSearch"/>
	</xsl:variable>
   
	<xsl:call-template name="buildTheSearchForm">
		<xsl:with-param name="formName" select="'searchCourseReserves'"/>
